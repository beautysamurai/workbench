...
- **Evidence:** F11 and Escape round-trip native fullscreen through an explicit normal-bounds snapshot. Workbench now also reconciles WSLg's unapplied fractional desktop scale before Electron becomes ready: on the reported 150% display, both the Windows surface and Electron renderer reached `(0,0) 2560×1440`, seven real pointer clicks retained exact y-coordinates, and the original 900×700 bounds restored. Model/reasoning choices remain keyed by Codex thread and were previously verified with two independent thread settings. The focused scale regression, all 13 test files, strict checks, production build, patched Electron launch, and real Windows-pointer journey passed.
