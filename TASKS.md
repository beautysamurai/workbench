...
- **Evidence:** The installed Codex catalog populates thread-scoped model and reasoning controls, while usage and reset time come from app-server responses and notifications. New workspaces safely initialize the Markdown workflow, and the task composer provides explicit P0–P3 priority, optional parent and acceptance-criteria fields, plus paste/drop/file image previews. Durable locked `WB-NNN` allocation prevents reuse and cross-process collisions, with descriptor-pinned counter candidates rejecting install-time rewrites; arbitrary legacy heading IDs remain visible and can parent new tasks when unambiguous, while a disappearing or invalid selected parent resets to top level during refresh; parent metadata renders as a semantic tree; bounded PNG/JPEG/WebP files stay under `.workbench/task-images/`; complete filter-reconstructed PNG data with in-range indexed palette samples, JPEG tables/scans, lossy VP8 control/token partitions, VP8L streams, and ordered single WebP alpha payloads are checked before acceptance; compressed PNG/JPEG/WebP work runs outside the Electron main thread; animated WebP frames must fit their declared canvas; workflow and metadata files must be direct single-link regular files inside the workspace; task reads/updates, locking, sequencing, image writes, and cleanup stay anchored to validated filesystem handles; temporary image inodes remain pinned through mode setting and installation; both installation rollback and later failure cleanup atomically claim the generated path and remove only an attachment whose pinned inode (plus mode, link count, and digest where available) still matches, preserving a concurrent replacement; a descriptor-revalidated atomic compare-and-update discards a modified task candidate, restores the prior file, retries compatible edits, and rejects invalidated parents; a committed Markdown update remains a successful add even if its follow-up refresh fails; and duplicate IDs remain visible without exposing an ambiguous Send action. Focused success, failure, concurrency, renderer, launch, strict-check, packaged-runtime, and production-build verification passed.
