...
async function reserveProjectTaskId(workspace: Workspace, tasks: ProjectTask[]): Promise<string> {
  ...
  const output = await runProjectScript(workspace, [
    ...
    'previous="$workbench_dir_fd/.task-sequence-previous.$$"',
    'rejected="$workbench_dir_fd/.task-sequence-rejected.$$"',
    'if [ -e "$temporary" ] || [ -L "$temporary" ] || [ -e "$previous" ] || [ -L "$previous" ] || [ -e "$rejected" ] || [ -L "$rejected" ]; then printf "Task id sequence temporary path already exists" >&2; exit 9; fi',
    'sequence_committed=0',
    'sequence_candidate_installed=0',
    'temporary_identity=',
    'temporary_fd=',
    'rollback_sequence_update() { if [ -n "$temporary_identity" ] && [ ! -L "$temporary" ] && [ -f "$temporary" ] && [ "$(stat -Lc "%d:%i" -- "$temporary" 2>/dev/null)" = "$temporary_identity" ]; then rm -f -- "$temporary" 2>/dev/null || true; fi; if [ "$sequence_committed" -eq 1 ]; then rm -f -- "$previous" 2>/dev/null || true; elif [ "$sequence_candidate_installed" -eq 1 ]; then rollback_counter_identity=$(stat -Lc "%d:%i" -- "$counter" 2>/dev/null || true); if [ -n "$temporary_identity" ] && [ ! -L "$counter" ] && [ -f "$counter" ] && [ "$rollback_counter_identity" = "$temporary_identity" ]; then mv -nT -- "$counter" "$rejected" 2>/dev/null || true; if [ -e "$rejected" ] || [ -L "$rejected" ]; then rejected_identity=$(stat -Lc "%d:%i" -- "$rejected" 2>/dev/null || true); if [ ! -L "$rejected" ] && [ -f "$rejected" ] && [ "$rejected_identity" = "$temporary_identity" ]; then rm -f -- "$rejected" 2>/dev/null || true; elif [ ! -e "$counter" ] && [ ! -L "$counter" ]; then mv -nT -- "$rejected" "$counter" 2>/dev/null || true; fi; fi; fi; fi; if [ "$sequence_committed" -ne 1 ] && { [ -e "$previous" ] || [ -L "$previous" ]; }; then if [ ! -e "$counter" ] && [ ! -L "$counter" ]; then mv -nT -- "$previous" "$counter" 2>/dev/null || true; else rollback_current_identity=$(stat -Lc "%d:%i" -- "$counter" 2>/dev/null || true); if [ "$sequence_candidate_installed" -ne 1 ] || [ -L "$counter" ] || [ ! -f "$counter" ] || [ "$rollback_current_identity" != "$temporary_identity" ]; then rm -f -- "$previous" 2>/dev/null || true; fi; fi; fi; }',
    'trap \'rollback_sequence_update\' EXIT',
    'umask 077',
    'set -C',
    'exec 3> "$temporary" || { set +C; printf "Task id sequence could not be reserved" >&2; exit 9; }',
    'set +C',
    'temporary_fd="/proc/$$/fd/3"',
    'temporary_identity=$(stat -Lc "%d:%i" -- "$temporary_fd" 2>/dev/null) || { printf "Task id sequence candidate is unsafe" >&2; exit 9; }',
    'if [ ! -f "$temporary_fd" ] || [ "$(stat -Lc %h -- "$temporary_fd" 2>/dev/null)" != 1 ]; then printf "Task id sequence candidate is unsafe" >&2; exit 9; fi',
    'printf "%s\\n" "$next" >&3 || { printf "Task id sequence could not be written" >&2; exit 9; }',
    'temporary_mode=$(stat -Lc %a -- "$temporary_fd" 2>/dev/null) || { printf "Task id sequence candidate is unsafe" >&2; exit 9; }',
    'expected_sequence_size=$((${#next} + 1))',
    'sequence_candidate_is_valid() { expected_sequence_links="$1"; if [ -z "$temporary_fd" ] || [ -z "$temporary_identity" ] || [ ! -f "$temporary_fd" ]; then return 1; fi; candidate_identity=$(stat -Lc "%d:%i" -- "$temporary_fd" 2>/dev/null) || return 1; candidate_links=$(stat -Lc %h -- "$temporary_fd" 2>/dev/null) || return 1; candidate_mode=$(stat -Lc %a -- "$temporary_fd" 2>/dev/null) || return 1; candidate_size=$(stat -Lc %s -- "$temporary_fd" 2>/dev/null) || return 1; candidate_value=$(cat -- "$temporary_fd") || return 1; [ "$candidate_identity" = "$temporary_identity" ] && [ "$candidate_links" = "$expected_sequence_links" ] && [ "$candidate_mode" = "$temporary_mode" ] && [ "$candidate_size" = "$expected_sequence_size" ] && [ "$candidate_value" = "$next" ]; }',
    'sequence_candidate_path_is_pinned() { [ ! -L "$temporary" ] && [ -f "$temporary" ] && [ "$(stat -Lc "%d:%i" -- "$temporary" 2>/dev/null)" = "$temporary_identity" ]; }',
    'if ! sequence_candidate_path_is_pinned || ! sequence_candidate_is_valid 1; then printf "Task id sequence candidate changed" >&2; exit 9; fi',
    'if [ "$counter_present" -eq 1 ]; then current_counter_hash=$(sha256sum -- "$counter_fd" 2>/dev/null) || { printf "Task id sequence changed during reservation" >&2; exit 9; }; current_counter_hash=${current_counter_hash%% *}; if [ -L "$counter" ] || [ ! -f "$counter" ] || [ "$(stat -Lc "%d:%i" -- "$counter" 2>/dev/null)" != "$counter_identity" ] || [ "$(stat -Lc %h -- "$counter_fd" 2>/dev/null)" != 1 ] || [ "$(stat -Lc %a -- "$counter_fd" 2>/dev/null)" != "$counter_mode" ] || [ "$current_counter_hash" != "$counter_hash" ]; then printf "Task id sequence changed during reservation" >&2; exit 9; fi; mv -nT -- "$counter" "$previous" || { printf "Task id sequence could not be claimed" >&2; exit 9; }; claimed_counter_hash=$(sha256sum -- "$counter_fd" 2>/dev/null) || { printf "Task id sequence changed during reservation" >&2; exit 9; }; claimed_counter_hash=${claimed_counter_hash%% *}; if [ -e "$counter" ] || [ -L "$counter" ] || [ -L "$previous" ] || [ ! -f "$previous" ] || [ "$(stat -Lc "%d:%i" -- "$previous" 2>/dev/null)" != "$counter_identity" ] || [ "$(stat -Lc %h -- "$counter_fd" 2>/dev/null)" != 1 ] || [ "$(stat -Lc %a -- "$counter_fd" 2>/dev/null)" != "$counter_mode" ] || [ "$claimed_counter_hash" != "$counter_hash" ]; then printf "Task id sequence changed during reservation" >&2; exit 9; fi; elif [ -e "$counter" ] || [ -L "$counter" ]; then printf "Task id sequence changed during reservation" >&2; exit 9; fi',
    'if ! sequence_candidate_path_is_pinned || ! sequence_candidate_is_valid 1; then printf "Task id sequence candidate changed" >&2; exit 9; fi',
    'if ! ln -LT -- "$temporary_fd" "$counter" 2>/dev/null; then printf "Task id sequence changed during reservation" >&2; exit 9; fi',
    'sequence_candidate_installed=1',
    'if ! sequence_candidate_path_is_pinned || ! sequence_candidate_is_valid 2 || [ -L "$counter" ] || [ ! -f "$counter" ] || [ "$(stat -Lc "%d:%i" -- "$counter" 2>/dev/null)" != "$temporary_identity" ] || [ "$(stat -Lc %h -- "$counter" 2>/dev/null)" != 2 ] || [ "$(stat -Lc %a -- "$counter" 2>/dev/null)" != "$temporary_mode" ] || [ "$(stat -Lc %s -- "$counter" 2>/dev/null)" != "$expected_sequence_size" ] || [ "$(cat -- "$counter")" != "$next" ]; then printf "Task id sequence candidate changed" >&2; exit 9; fi',
    'rm -f -- "$temporary" || { printf "Task id sequence candidate could not be removed" >&2; exit 9; }',
    'if [ -e "$temporary" ] || [ -L "$temporary" ] || ! sequence_candidate_is_valid 1 || [ -L "$counter" ] || [ ! -f "$counter" ] || [ "$(stat -Lc "%d:%i" -- "$counter" 2>/dev/null)" != "$temporary_identity" ] || [ "$(stat -Lc %h -- "$counter" 2>/dev/null)" != 1 ] || [ "$(stat -Lc %a -- "$counter" 2>/dev/null)" != "$temporary_mode" ] || [ "$(stat -Lc %s -- "$counter" 2>/dev/null)" != "$expected_sequence_size" ] || [ "$(cat -- "$counter")" != "$next" ]; then printf "Task id sequence candidate changed" >&2; exit 9; fi',
...
async function writeProjectTaskImage(
  ...
  const writtenIdentity = (await runProjectScript(workspace, [
    ...
    `rejected="$image_dir_fd/.${filename}.workbench-rejected.$$"`,
    'if [ -e "$temporary" ] || [ -L "$temporary" ] || [ -e "$rejected" ] || [ -L "$rejected" ]; then printf "Task image temporary path already exists" >&2; exit 8; fi',
    'image_installed=0',
    'temporary_identity=',
    'rollback_task_image_install() { if [ "$image_installed" -ne 1 ] || { [ ! -e "$target" ] && [ ! -L "$target" ]; }; then return 0; fi; if [ -e "$rejected" ] || [ -L "$rejected" ]; then printf "Task image rollback path already exists" >&2; return 1; fi; mv -nT -- "$target" "$rejected" 2>/dev/null || { printf "Task image could not be claimed for rollback" >&2; return 1; }; if [ ! -e "$rejected" ] && [ ! -L "$rejected" ]; then printf "Task image could not be claimed for rollback" >&2; return 1; fi; rejected_identity=$(stat -c "%d:%i" -- "$rejected" 2>/dev/null || true); if [ ! -L "$rejected" ] && [ -f "$rejected" ] && [ -n "$temporary_identity" ] && [ "$rejected_identity" = "$temporary_identity" ]; then rm -f -- "$rejected" 2>/dev/null || { printf "Task image rollback failed" >&2; return 1; }; elif [ ! -e "$target" ] && [ ! -L "$target" ]; then mv -nT -- "$rejected" "$target" 2>/dev/null || true; if [ -e "$rejected" ] || [ -L "$rejected" ]; then printf "Task image changed during rollback" >&2; return 1; fi; else printf "Task image changed during rollback" >&2; return 1; fi; }',
    'cleanup_task_image_install() { rollback_task_image_install || true; if [ -n "$temporary_identity" ] && [ ! -L "$temporary" ] && [ -f "$temporary" ] && [ "$(stat -Lc "%d:%i" -- "$temporary" 2>/dev/null)" = "$temporary_identity" ]; then rm -f -- "$temporary" 2>/dev/null || true; fi; }',
    'trap \'cleanup_task_image_install\' EXIT',
    'umask 077',
    'set -C',
    ...
    'image_installed=1',
    'target_identity=$(stat -Lc "%d:%i" -- "$target" 2>/dev/null) || { printf "Task image installation is unsafe" >&2; exit 8; }',
    'if [ -L "$target" ] || [ ! -f "$target" ] || [ "$target_identity" != "$temporary_identity" ]; then printf "Task image installation changed during validation" >&2; exit 8; fi',
    'rm -f -- "$temporary" || { printf "Task image temporary file could not be removed" >&2; exit 8; }',
    'if [ "$(stat -Lc "%d:%i" -- "$temporary_fd" 2>/dev/null)" != "$temporary_identity" ] || [ "$(stat -Lc %h -- "$temporary_fd" 2>/dev/null)" != 1 ] || [ "$(stat -Lc "%d:%i" -- "$target" 2>/dev/null)" != "$temporary_identity" ]; then printf "Task image installation changed during validation" >&2; exit 8; fi',
  ...
  for (let attempt = 0; attempt < PROJECT_TASK_COMMIT_ATTEMPTS; attempt += 1) {
    ...
    try {
      ...
      await runProjectScript(workspace, [
        ...
        `rejected="$root_fd/.TASKS.md.workbench-rejected-${taskId}.$$"`,
        'if [ -e "$candidate" ] || [ -L "$candidate" ] || [ -e "$claim" ] || [ -L "$claim" ] || [ -e "$rejected" ] || [ -L "$rejected" ]; then printf "Task update temporary path already exists" >&2; exit 5; fi',
        'committed=0',
        'candidate_installed=0',
        'candidate_identity=',
        'candidate_fd=',
        'claim_fd=',
        'rollback_task_update() { if [ -n "$candidate_identity" ] && [ ! -L "$candidate" ] && [ -f "$candidate" ] && [ "$(stat -Lc "%d:%i" -- "$candidate" 2>/dev/null)" = "$candidate_identity" ]; then rm -f -- "$candidate" 2>/dev/null || true; fi; if [ "$committed" -eq 1 ]; then rm -f -- "$claim" 2>/dev/null || true; elif [ "$candidate_installed" -eq 1 ]; then rollback_target_identity=$(stat -Lc "%d:%i" -- "$target" 2>/dev/null || true); if [ -n "$candidate_identity" ] && [ ! -L "$target" ] && [ -f "$target" ] && [ "$rollback_target_identity" = "$candidate_identity" ]; then mv -nT -- "$target" "$rejected" 2>/dev/null || true; if [ -e "$rejected" ] || [ -L "$rejected" ]; then rejected_identity=$(stat -Lc "%d:%i" -- "$rejected" 2>/dev/null || true); if [ ! -L "$rejected" ] && [ -f "$rejected" ] && [ "$rejected_identity" = "$candidate_identity" ]; then rm -f -- "$rejected" 2>/dev/null || true; elif [ ! -e "$target" ] && [ ! -L "$target" ]; then mv -nT -- "$rejected" "$target" 2>/dev/null || true; fi; fi; fi; fi; if [ "$committed" -ne 1 ] && { [ -e "$claim" ] || [ -L "$claim" ]; }; then if [ ! -e "$target" ] && [ ! -L "$target" ]; then if [ -n "$claim_fd" ] && [ -f "$claim_fd" ] && ln -LT -- "$claim_fd" "$target" 2>/dev/null; then rm -f -- "$claim" 2>/dev/null || true; else mv -nT -- "$claim" "$target" 2>/dev/null || true; fi; else rollback_current_identity=$(stat -Lc "%d:%i" -- "$target" 2>/dev/null || true); if [ "$candidate_installed" -ne 1 ] || [ -L "$target" ] || [ ! -f "$target" ] || [ "$rollback_current_identity" != "$candidate_identity" ]; then rm -f -- "$claim" 2>/dev/null || true; fi; fi; fi; }',
