...
[10:00:00.001] rdpMonitor[0]: desktopScaleFactor:100, deviceScaleFactor:100
...
test('parses the primary Windows desktop scale from the latest WSLg layout input', () => {
  const staleLayout = mixedScaleLayout.replaceAll('150', '200');
  assert.equal(parseWslgPrimaryDisplayScale(`${staleLayout}\n${mixedScaleLayout}`), 1.5);
  assert.equal(
    parseWslgPrimaryDisplayScale(
      mixedScaleLayout.replace('desktopScaleFactor:150', 'desktopScaleFactor:200')
        .replaceAll('client scale :1.00', 'client scale :2.00'),
    ),
    1,
  );
  assert.equal(parseWslgPrimaryDisplayScale('desktopScaleFactor:150'), null);
  assert.equal(parseWslgPrimaryDisplayScale(mixedScaleLayout.replace('150', '600')), null);
});
